class GridHelper:
    ...
    def fill_polygon(self, canvas, pos, pts, color=None):
        ...
        '''
        canvas.save()
        x0, y0 = self.cr2xy(*pos)
        pts = [(x0+x*self.dx, y0+y*self.dy) for x, y in pts]
        if color:
            canvas.fill_style = color
    ...
    def fill_text(self, canvas, text, pos, color=None, margin=0.1):
        ...
        canvas.font = f'{(1-2*margin)*self.dy}px sans-serif'
        if color:
            canvas.fill_style = color

        canvas.fill_text(text, x, y, max_width=self.dx)
        canvas.restore()
