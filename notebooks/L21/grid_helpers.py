...
def get_rect(pos, grid_spec):
    '''liefert (x0, y0, dx*ncol, dy*nrow)'''
    x0, y0, dx, dy = grid_spec[:4]
    x, y = x0+pos[0]*dx, y0+pos[1]*dy
    pts = [x, y, x+dx, y+dy]
    ix, iy, ix_, iy_ = [round(pt) for pt in pts]
    return ix, iy, ix_-ix, iy_-iy


def get_grid_rect(grid_spec):
    '''liefert (x0, y0, dx*ncol, dy*nrow)'''
    x0, y0, dx, dy, ncol, nrow = grid_spec
...
def fill_rect(canvas, pos, grid_spec, color=None):
    ...
    canvas.fill_rect(*get_rect(pos, grid_spec))
...
def stroke_rect(canvas, pos, grid_spec, color=None, line_width=None):
    ...
    if line_width:
        canvas.line_width = line_width
    canvas.stroke_rect(*get_rect(pos, grid_spec))


def clear_rect(canvas, pos, grid_spec):
    '''loescht das Gitterfeld pos=(col, row)'''
    canvas.clear_rect(*get_rect(pos, grid_spec))
