...
def draw_layer(canvas, ss, pos, scale, m=False):
    pts_ = ((-1, -1), (-4/3, 0), (0, -4/3))
    pts = [(x, -y) for x, y in pts_] if m else pts_
    x0, y0 = pos

    for a, b, c in ss[:4]:
        ptsc = [(x0 + scale*x, y0 + scale*y) for x, y in pts]
        (corner_x, corner_y), (side1_x, side1_y), (side2_x, side2_y) = ptsc

        canvas.fill_style = colors[a]
        canvas.fill_rect(corner_x, corner_y, x0-corner_x, y0 - corner_y)

        canvas.fill_style = colors[b]
        ...
        pts = [rot90(pt, not m) for pt in pts]
...
def draw_cube(canvas, s):
    canvas.clear()
    ss = [stickers[i][-j:] + stickers[i][:-j] for i, j in zip(s[:8], s[8:])]
    w, h = canvas.width, canvas.height

    with hold_canvas(canvas):
        draw_layer(canvas, ss[:4], pos=(w//4, h//2), scale=h//5)
        draw_layer(canvas, ss[4:], pos=(3*w//4, h//2), scale=h//5, m=True)
