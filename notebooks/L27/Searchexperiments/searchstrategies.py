...
def search_df(node, get_neighbors, goal):
    ...
    while nodes_to_visit:
        ...
        if node == goal:
            return goal, go_back, nodes_to_visit, d_dict

        for neighbor in get_neighbors(node):
            if neighbor in go_back:
                continue
            go_back[neighbor] = node
            d_dict[neighbor] = d_dict[node] + 1
            nodes_to_visit.append(neighbor)

    return node, go_back, nodes_to_visit, d_dict
...
def search_greedy(node, get_neighbors, h, goal):
    ...
    while nodes_to_visit:
        ...
        if node == goal:
            ...
            return goal, go_back, front, p_dict
...
def search_bibf(node, get_neighbors, goal):
    ...
    while nodes_to_visit_1 and nodes_to_visit_2:
        for i in (0, 1):
            if (node := _search(i)):
                return node, go_backs, deques, d_dicts

    return None, go_backs, deques, d_dicts
...
def search_bi_smart(node, get_neighbors, hs, goal):
    ...
    p_dicts = ({node: hs[0](node)}, {goal: hs[1](goal)})
    go_back_1 = {node: None}
    go_back_2 = {goal: None}
    ...
    while nodes_to_visit_1 and nodes_to_visit_2:
        for i in (0, 1):
            if (node := _search(i)):
                ...
                return node, go_backs, fronts, p_dicts

    fronts = tuple([x[2] for x in heap] for heap in heaps)
    return None, go_backs, fronts, p_dicts